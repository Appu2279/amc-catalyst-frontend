...
export const AdminMockTestDetail = () => {
  ...
  const [subjects, setSubjects]       = useState([]);
  const [subjectCounts, setSubjectCounts] = useState({}); // { subject_id: count }
  ...
  const loadTest = useCallback(async () => {
    try {
      const [testRes, subRes] = await Promise.all([getMockTest(id), getSubjects()]);
      const testData = testRes.data?.data ?? testRes.data;
      setTest(testData);
      setSubjects(subRes.data?.data ?? subRes.data ?? []);

      // For dynamic tests, fetch actual question counts per subject
      if (testData?.test_type === 'dynamic' && testData?.configuration_json?.subjects?.length) {
        const counts = {};
        await Promise.all(
          testData.configuration_json.subjects.map(async ({ subject_id }) => {
            try {
              const r = await getQuestionsAdmin({ subject_id, limit: 1, is_active: true });
              counts[subject_id] = r.data?.pagination?.total ?? 0;
            } catch { counts[subject_id] = 0; }
          })
        );
        ...
        {isDynamic && (
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => navigate('/admin/mock-tests', { state: { editId: test.id } })}
            ...
            {cfg.subjects?.length > 0 ? (
              <div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="text-center px-4 py-2 text-xs font-semibold text-slate-500">Needs</th>
                        <th className="text-center px-4 py-2 text-xs font-semibold text-slate-500">Available</th>
                        <th className="text-center px-4 py-2 text-xs font-semibold text-slate-500">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {cfg.subjects.map((s, i) => {
                        const subjectName = subjects.find(sub => String(sub.id) === String(s.subject_id))?.name;
                        const available   = subjectCounts[s.subject_id] ?? '…';
                        const ok          = typeof available === 'number' && available >= s.count;
                        const notFound    = !subjectName;
                        return (
                            ...
                            <td className="px-4 py-2.5">
                              {notFound ? (
                                <span className="flex items-center gap-1.5 text-red-600 font-medium">
                                  ...
                                  ID {s.subject_id} not found
                                </span>
                              ) : (
                                <span className="text-slate-700 font-medium">{subjectName}</span>
                              )}
                              <span className="block text-[10px] text-slate-400 mt-0.5">ID: {s.subject_id}</span>
                            </td>
                            <td className="px-4 py-2.5 text-center text-slate-600 font-medium">{s.count}</td>
                            <td className="px-4 py-2.5 text-center font-medium">
                              <span className={typeof available === 'number' ? (available >= s.count ? 'text-green-600' : 'text-red-500') : 'text-slate-400'}>
                                {available}
                              </span>
                            </td>
                            <td className="px-4 py-2.5 text-center">
                              {notFound || (typeof available === 'number' && !ok) ? (
                                <span className="inline-flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                                  <AlertTriangle className="w-3 h-3" /> Fix needed
                                </span>
                              ) : typeof available === 'number' && ok ? (
                ...
                {/* Call-to-action if any row has issues */}
                {cfg.subjects.some(s => {
                  const notFound  = !subjects.find(sub => String(sub.id) === String(s.subject_id));
                  const available = subjectCounts[s.subject_id];
                  return notFound || (typeof available === 'number' && available < s.count);
                }) && (
                  <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
                    ...
                    <div>
                      <p className="font-semibold">Subject mismatch detected</p>
                      <p className="text-xs mt-0.5 text-amber-700">
                        One or more configured subjects don't exist or have fewer questions than required.
                        Click <strong>Edit Config</strong> above and re-select the correct subjects.
                      </p>
                    </div>
                  </div>
                )}
              ...
              <p className="text-sm text-slate-400">No subjects configured.</p>
            )}
